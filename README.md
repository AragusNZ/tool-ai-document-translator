...
| `--config PATH` | JSON file with `PipelineConfig` overrides and optional `export_format` / `target_lang` / `source_lang` / `translation_context` / `translation_mode` / `pdf_ocr` / `job_timeout_seconds` / `webhook_url` / `webhook_secret` |
